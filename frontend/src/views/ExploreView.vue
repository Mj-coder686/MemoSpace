...
<template>
  ...
  <div v-else class="space-grid">
    <article v-for="person in people" :key="person.id" class="panel profile-hero" style="grid-template-columns:auto 1fr">
      <router-link :to="`/user/${person.id}`"><UserAvatar class="profile-avatar" style="width:58px;height:58px;font-size:21px" :src="person.avatar" :name="person.nickname" /></router-link>

...
const chooseAvatar=(event:Event)=>{const file=(event.target as HTMLInputElement).files?.[0]||null;avatarFile.value=file;if(avatarPreview.value)URL.revokeObjectURL(avatarPreview.value);avatarPreview.value=file?URL.createObjectURL(file):''}
  ...
  <div class="settings-layout">
    <div class="settings-main">
      ...
      <section class="panel profile-editor">
        ...
        <label class="profile-avatar-picker"><Upload :size="18" /><span><b>{{avatarFile?.name||'上传新头像'}}</b><small>支持 JPEG、PNG、WebP 或 GIF，保存后好友也能看到。</small></span><input type="file" accept="image/*" @change="chooseAvatar" /></label>
